...
async def save_accounts(user_id, accounts):
    ...
    print(f"DEBUG save_accounts: user_id={user_id}, saving {len(accounts_to_save)} accounts")
    result = await db.users.update_one(
        {"user_id": user_id},
        {"$set": {"accounts": accounts_to_save}},
        upsert=True
    )
    print(f"DEBUG save_accounts: matched={result.matched_count}, modified={result.modified_count}")
...
async def load_owner_accounts():
    if not OWNER_IDS:
        print("DEBUG load_owner_accounts: OWNER_IDS is empty!")
        return []
    first_owner = next(iter(OWNER_IDS))
    print(f"DEBUG load_owner_accounts: loading for owner ID {first_owner}")
    accounts = await load_accounts(first_owner)
    print(f"DEBUG load_owner_accounts: found {len(accounts)} accounts")
    return accounts
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_owner(user_id):
        ...
        print(f"DEBUG show_main_menu: owner_accounts count = {len(owner_accounts)}")
        active_count = 0
...
async def admin_remove_all_expired(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await save_accounts(OWNER_ID, new_accounts)  # sync to first owner? but we have multiple owners - maybe we should remove from all? For simplicity, we only remove from first owner's list. Actually we need to remove from all owners' lists. But the function save_accounts only saves for given user_id. We'll manually remove from all owners.
    # Better: iterate over all owners and remove expired accounts
    for owner_id in OWNER_IDS:
        owner_accs = await load_accounts(owner_id)
        new_owner_accs = [a for a in owner_accs if a.get('phone') not in expired_phones]
        await save_accounts(owner_id, new_owner_accs)

    # Also remove from non-owner users
    users = await get_all_users()
    for u in users:
        uid = u[0]
...
async def admin_remove_account(update: Update, context: ContextTypes.DEFAULT_TYPE, phone: str):
    ...
    # Remove from all owners' accounts
    found = False
    for owner_id in OWNER_IDS:
        owner_accounts = await load_accounts(owner_id)
        if any(a.get('phone') == phone for a in owner_accounts):
            found = True
            new_owner_accounts = [a for a in owner_accounts if a.get('phone') != phone]
            await save_accounts(owner_id, new_owner_accounts)

    if not found:
        await update.message.reply_text("❌ Account not found in owner accounts!")
        WAITING_FOR.pop(user_id, None)
        return
    ...
    WAITING_FOR.pop(user_id, None)
    await update.message.reply_text(f"✅ Account {phone} removed from all owners and users!")
...
async def admin_remove_all_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Count total accounts across all owners
    total_accounts = 0
    for owner_id in OWNER_IDS:
        owner_accs = await load_accounts(owner_id)
    ...
    reply_markup = {"inline_keyboard": [[dict(btn) for btn in row] for row in keyboard]}
    await update.callback_query.edit_message_text(
        f"⚠️ WARNING!\n\nRemove ALL {total_accounts} accounts from all owners?\n\nThis cannot be undone!",
        reply_markup=reply_markup)
...
async def admin_remove_all_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Clear accounts for all owners
    for owner_id in OWNER_IDS:
        await save_accounts(owner_id, [])

    # Clear accounts for all non-owner users
    users = await get_all_users()
    for u in users:
        uid = u[0]
        if uid not in OWNER_IDS:
...
async def main():
    ...
    # Clear any existing webhook to prevent Conflict errors
